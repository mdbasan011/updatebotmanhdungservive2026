...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_join(ctx, user.id):
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("2️⃣  Tham gia Nhóm 2", url=f"https://t.me/{REQUIRED_GROUPS[2].lstrip('@')}")],
...
async def cmd_add(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(ctx.args) < 2:
        return await get_reply(update).reply_text(
            ...
            "│\n"
            "│  VD: /add 123456789 7d\n"
            "│  VD: /add 123456789 1w2d\n"
            "│  VD: /add 123456789 12h30m\n"
            "└──────────────────\n"
            "```",
            parse_mode=ParseMode.MARKDOWN_V2
        )

    target_uid = ctx.args[0].strip()
